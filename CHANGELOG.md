...
- Fix: opening the sidebar panel no longer triggers an automatic usage fetch. Data is shown from cache (stale) if available, or the unauthenticated state otherwise. Fetches now only happen on manual refresh or via the auto-refresh timer.
